...
def generate_step(
    ...
    *,
    ...
    Args:
        ...
        prompt_cache_tokens (mx.array, optional): Token ids already represented by
          ``prompt_cache`` and omitted from ``prompt``.  When logits processors are
          enabled, these ids are prepended to the prompt history seen by each
          processor.
    ...
    # Keep an unmodified view of the caller's prompt.  The prefill loop below
    # slices ``prompt`` in place, while logits processors need the complete
    # logical prompt (including a cached prefix) on their first invocation.
    ...
    def _step(input_tokens: mx.array, input_embeddings: Optional[mx.array] = None):
        with mx.stream(generation_stream):
            ...
            if logits_processors and len(input_tokens) > 0:
                tokens = token_buffer.update_and_fetch(input_tokens)
        ...
        with mx.stream(generation_stream):
            ...
            if logits_processors:
                ...
                for i in range(n_predict):
                    tokens = token_buffer.update_and_fetch(y)
    ...
    try:
        while True:
            ...
            if token_buffer is not None:
                # Draft calls append their input tokens to the processor
                # history.  Restore the committed target history before
                # verification, removing exactly the temporary draft inputs.
                token_buffer.trim(y.size + num_draft - 1)
            ...
            if token_buffer is not None:
                # Keep the target-sampled prefix that corresponds to accepted
                # drafts.  The extra trim for an all-accepted round removes the
                # final target token, which is carried into the next draft step.
                token_buffer.trim(max(num_draft - n, 1))
